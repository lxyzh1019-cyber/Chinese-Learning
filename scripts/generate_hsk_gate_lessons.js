#!/usr/bin/env node
/**
 * Regenerate data/lessons/hsk{N}_gate_XX.json from data/hsk{N}.json (N=1..4).
 * Usage: node scripts/generate_hsk_gate_lessons.js 3
 */
"use strict";
...
function buildLesson(level, gate) {
  const gid = gate.gateId;
  const theme = gate.theme || `HSK${level} Gate ${gid}`;
  ...
  const newClosing = newN > 10 ? `……等（共${newN}个生字）。` : `（共${newN}个生字）。`;
  ...
  return {
    lessonId: `hsk${level}_gate_${String(gid).padStart(2, "0")}`,
    level: `HSK${level}`,
    gateId: gid,
    title: `HSK${level} Gate ${gid} Lesson`,
    explanation,
    passage,
    keyVocab,
    comprehension: [
...
function main() {
  ...
  if (![1, 2, 3, 4].includes(level)) {
    console.error("Usage: node scripts/generate_hsk_gate_lessons.js <1|2|3|4>");
    process.exit(1);
  }
  const src = path.join(ROOT, "data", `hsk${level}.json`);
  const doc = JSON.parse(fs.readFileSync(src, "utf8"));
  if (!Array.isArray(doc.gates) || doc.gates.length !== 22) {
    throw new Error(`hsk${level}.json: expected 22 gates`);
  }
  ...
  for (const gate of doc.gates) {
    ...
    const lesson = buildLesson(level, gate);
    const fname = `hsk${level}_gate_${String(gid).padStart(2, "0")}.json`;
