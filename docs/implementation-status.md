...
|---|---|---|---|---|
| P0-1 | Reconcile audited baseline with HEAD | verified | — | The 2026-09-08 re-audit is against `e1f097b`, which was HEAD when it ran. No drift. |
...
| ID | Requirement | Status | Files | Evidence |
|---|---|---|---|---|
| A26-R04 | Review history merges losslessly past the 40-attempt bound | verified | `js/review-core.js`, `js/merge-state.js` | Trimming discarded the overflow, so `mergeRecords` abandoned replay and kept one whole copy: order-dependent (both call sites pass local first, so devices never converged) and lossy with unequal lengths too — 40 old entries beat three fresh misses. Trimming now folds into a checkpoint and the union-and-replay path always runs. 4 new tests fail against the previous code; the old bound test asserted the removed behaviour and was replaced. |
...
| A10 | Attempt persistence, revision CAS, offline queue | verified | `js/player-store.js` | A-T11 plus queue/append tests. **Demoted then re-verified (2026-09-08 audit A26-R01/R02).** Every test awaited each call, so no upload was ever in flight while the attempt changed: the acknowledgement recorded a revision the cloud had never seen, and a save made during a flush was dropped from the queue. Now: payload and revision snapshotted before any await, uploads serialized per attempt, queue entries removed only by the revision actually uploaded, a base ahead of the cloud re-based. 5 new tests hold the write open; 3 fail against the previous code. |
...
| R02 | Next-day → +3 → +7 → +14 → +30, on unaided recall only | verified | `js/review-core.js` | **Demoted then re-verified (2026-09-08 audit A26-R03).** The retry flag was never persisted, so a child shown the answer, interrupted, and returning had the answer they were told recorded as independent recall. Held only within one uninterrupted sitting, which is where the tests looked. Now persisted and restored; 4 new tests fail against the previous code. Ladder walked step by step; a supported answer or a retry after the reveal never advances it; four taps in one sitting advance once; a miss returns to tomorrow without forfeiting past successes. Calendar arithmetic checked across a month end and a leap day. |
...
| F06a — the F06 fix never took effect, and lessons asked twice | `index.html` (`[hidden]` CSS rule, `.lesson-ans`/`.lesson-verdict` classes, `renderGateLesson` question gating) | 4 in `tests/progression.test.cjs` ("F06:" hiddenness computed, "A26:" one set of questions, old template) | The answer element carried `hidden` AND an inline `display:block`, which beats the UA `[hidden]` rule, so the answer was visible from first paint and Reveal only hid its own button. The page now ships `[hidden]{display:none!important}`. A lesson with checked questions asks only those; a lesson still on the pre-rewrite template (HSK3/HSK4) names the gap instead of asking an unreadable question. | Chromium against the real lesson files: computed `display` none -> block on Reveal, verdicts none -> flex, zero answer elements on a checked lesson, gap note on HSK3. |
